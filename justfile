...
plugins:
	...
	standard_plugin_list=(simple-cloudflare-turnstile wp-statistics all-in-one-seo-pack featured-image-with-url google-site-kit w3-total-cache)
...
wordpress: && plugins themes
	...
	if [ ! -f "wordpress/index.php" ]; then
	    if [ ! -f "temp/wordpress.zip" ]; then
	       wget -O temp/wordpress.zip https://cn.wordpress.org/latest-zh_CN.zip
	    fi
		unzip -q temp/wordpress.zip
		mv temp/wordpress .
