...
![模块](https://image.hyosakura.com/file/frangipani/IZiTbsFtToglV9xlnXFcWJOvn9g.png)
...
![存储](https://image.hyosakura.com/file/frangipani/LthSbYvskoP5eoxGWAsc6lMqnFf.png)
...
![缓存](https://image.hyosakura.com/file/frangipani/XPI5bUOnyol8Xfx6TqDchPj3n9w.png)

## 挑战

> Frangipani 的挑战主要来自于两方面，一个是缓存，另一个是这种去中心化的架构带来的大量的逻辑存在于客户端之中进而引起的问题

![挑战](https://image.hyosakura.com/file/frangipani/Ue01bZXBRobNGPxKuk4cLgZNnmd.png)

### 缓存一致性

假设工作站 W1 创建了一个文件 `/A`。最初，这个文件只会在本地缓存中创建。首先，`Frangipani` 需要从 Petal 获得 `/` 目录下的内容，之后当创建文件时，工作站只是修改缓存的拷贝，并不会将修改立即返回给 `Petal`。

![缓存一致性](https://image.hyosakura.com/file/frangipani/Gh9jbyh4roQvZ9xj6ENcRGalnGb.png)
...
![原子性](https://image.hyosakura.com/file/frangipani/Lmobbqyq3o1E9ExN3mHcbV2jn5d.png)
...
![锁服务器](https://image.hyosakura.com/file/frangipani/MZWVbN7lcoyNeGxxrG6cWedunwc.png)
...
![排他锁](https://image.hyosakura.com/file/frangipani/NCgxbQcEko4keQxm2mtchD7tnjQ.png)

在每个工作站，会记录跟踪它所持有的锁，和锁对应的文件内容。所以在每个工作站中，`Frangipani` 模块也会有一个 `lock` 表单，表单会记录文件名、对应的锁的状态和文件的缓存内容。这里的文件内容可能是大量的数据块，也可能是目录的列表。

![锁表单](https://image.hyosakura.com/file/frangipani/DUZrbj3S0o3ZoixOalTcy8uUnYc.png)
...
![busy](https://image.hyosakura.com/file/frangipani/QuE6bVL85opCm7xg2X4csrSRnuf.png)
...
![idle](https://image.hyosakura.com/file/frangipani/EqMZbEyNBo6OIuxTzNvcjQIZn2c.png)
...
![获取锁](https://image.hyosakura.com/file/frangipani/SAvqbNGPGoNazWxQilIcno85ndE.png)

- 如果你在释放锁之前，修改了锁保护的数据，那你必须将修改了的数据写回到 `Petal`，只有在 `Petal` 确认收到了数据，你才可以释放锁，也就是将锁归还给锁服务器。所以这里的顺序是，先向 `Petal` 存储系统写数据，之后再释放锁。

![释放锁](https://image.hyosakura.com/file/frangipani/U45qbEJLNoEmkJx0Xj6cSBuanpc.png)

最后再从工作站的 lock 表单中删除关文件的锁的记录和缓存的数据。

![删除表单项](https://image.hyosakura.com/file/frangipani/Cinbb9PBFoOLIMxw0UQcDloOnVh.png)
...
![异步request和grant](https://image.hyosakura.com/file/frangipani/W4WqbD2sIoGDOuxU6iUcRgQVnie.png)
...
![revoke](https://image.hyosakura.com/file/frangipani/J6RDbfTxioszdSxBM1ochIbRn0b.png)
...
![release](https://image.hyosakura.com/file/frangipani/K0rfbSVvMo5YjLxhKNacYJAPnKg.png)
...
![](https://image.hyosakura.com/file/frangipani/AOB5bcNygoo66IxG1WWcFdR2nle.png)

按照协议，如果 WS1 想要读取并修改文件 Z。在它从 Petal 读取文件之前，它需要先获取对于 Z 的锁，所以它向锁服务器发送 `Request` 消息（下图中 ACQ Z）。

![](https://image.hyosakura.com/file/frangipani/FqzubxzPUoiMzIxes9tct0RknLc.png)
...
![](https://image.hyosakura.com/file/frangipani/UMz0bImIhoeB6NxzBxrcueCnncg.png)
...
![](https://image.hyosakura.com/file/frangipani/SsjHbFNxToedzVxj6IgcQVCrnGe.png)

锁服务器必然会有一个表单记录谁在等待文件 Z 的锁，一旦锁的当前持有者释放了锁，锁服务器需要通知等待者。所以当锁服务器收到了这条 Release 消息时，锁服务器会更新自己的表单，并最终将 Grant 消息发送给工作站 WS2。

![](https://image.hyosakura.com/file/frangipani/VOh2bIlRRoCL7hxNi6Ucs7EQnFe.png)

这个时候，WS2 终于可以从 Petal 读取文件 Z。

![](https://image.hyosakura.com/file/frangipani/OCV8bLP24oovosx02Mbc1Bc4nih.png)
...
![WAL](https://image.hyosakura.com/file/frangipani/UJnFb4SrtoYt8uxDlNRcTfoGnCb.png)
...
![日志条目](https://image.hyosakura.com/file/frangipani/FdDGbS6xqoDBSnx4fjTcBpUsnWb.png)
...
![完整过程](https://image.hyosakura.com/file/frangipani/PKHZbwUP1orEoNxo1WRcx9Lynag.png)
...
![](https://image.hyosakura.com/file/frangipani/HCPdbRLNioXRN6x4SWocG23JnDh.png)
...
![](https://image.hyosakura.com/file/frangipani/UZCxbQScxosfdBxdJ0ZcYAE9n4E.png)
...
![](https://image.hyosakura.com/file/frangipani/N3VzbaRVzob0DJxXoaTcLHOonFc.png)
...
![](https://image.hyosakura.com/file/frangipani/Rj5xbQytgoBXCWxkzV5cDLGFn6b.png)
