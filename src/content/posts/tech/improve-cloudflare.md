...
image: https://gh-proxy.com/https://github.com/xiyan520/tp/blob/master/pc/163.jpg
tags: [cdn, cloudflare]
category: 技术
draft: false
---
...
该方法的有点就是配置简单，快！缺点就是它不支持多个回退源，且容易踩坑。大致方法跟双域名是一样的，只是回退源的域名跟用户访问主域名都是同一个。以 `test.com` 为例：

1. 你的用户访问(主)域名**必须**托管 Cloudflare ，但是子域名取决于你还想不想分地区使用不同的加速方式：
   1. 如果你想，将三级域名或者说子域名托管在支持**子域名接入** + **分地区解析**的服务商上，子域名此时的记录是 NS
   2. 如果不想，忽略这一步
2. 设置回退域名，一般该域名是你的闲置域名，这里以 `fallback.test.com` 作为回退域名为例，设置名为 fallback 的 A 记录作为回退域名，同时**开启**小黄云。那么此时就是 `fallback.test.com` 指向我的后端 ip。
3. 在 **`test.com`** 的设置界面，在 SSL/TLS -> 自定义主机名栏目中添加回退源和自定义主机名：
   1. 点设置回退源，这个即为上一步配置过小黄云的回退域名
   2. 点添加自定义主机名，主机名即为用户访问域名 `www.test.com`，自定义源服务器这里非常重要了，**<u>一定不能选择「默认源服务器」，必须选择「自定义源服务器」，否则会报错，这是与双域名优选的最大区别！！！</u>**
4. 配置 CNAME 解析：
   1. 如果选择了分地区解析：
      1. 国内用户，需要 CNAME 到加速域名(不开小黄云)，我常用的是 `saas.sin.fan`
      2. 海外用户，需要 CNAME 到回退域名，即 `fallback.test.com`
   2. 如果只配置了 Cloudflare 
      1. 在 Cloudflare 无脑添加 CNAME 记录到 到加速域名(不开小黄云)
